...
namespace ConsoleApp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            custDataType = BuildDynamicTypeWithProperties();
            ...
            // 속성을 호출 할 때 적절한 BindingFlags를 사용해야합니다.
            // "set"비헤이비어를 호출 할 때 BindingFlags.SetProperty
            // "get"비헤이비어를 호출 할 때 BindingFlags.GetProperty. 또한
            // 예상대로 속성에 지정한 이름을 기반으로 호출합니다.
            // 특정 속성 동작에 바인딩 된 메서드의 이름입니다.

            object custData = Activator.CreateInstance(custDataType);
            //List<custData> lists = new cu
            custDataType.InvokeMember("CustomerName", BindingFlags.SetProperty, null, custData, new object[] { "Joe User" });
